...
class Firstscene extends Phaser.Scene {
    ...
    preload() {
        ...
        this.load.image('background', 'backgrounds/background.png')
            ...
            .image("enemigo2", "sprites/enemigo2.png")
            ...
            .image('barrera', 'sprites/barrera.png');  // Reemplaza con tu imagen de la barrera;
    ...
    reloadAmmo() {
        if (this.ammo === 0) {
            ...
            this.cajaImage.setX(randomX).setActive(false).setVisible(false);
    ...
    fire(player, bulletGroup) {
        if (this.ammo >= 1) {
            ...
            bullet.lifespan = 2000; // Duración de la bala
            this.shotSound.play();
            this.ammo--;
            this.ammoText.setText('AMMO: ' + this.ammo);
        }
    ...
    addEnemy() {
        ...
        if (type === 0) {
            const enemy = this.enemigo2Group.create(Phaser.Math.Between(0, this.sys.game.canvas.width), 0, 'enemigo1');
            enemy.setVelocityY(60);
            enemy.hitsToKill = 2; // Define cuántos golpes necesita el enemigo para ser destruido
        } else if (type === 1) {
            const enemy = this.enemigo1Group.create(Phaser.Math.Between(0, this.sys.game.canvas.width), 0, 'enemigo2');
            enemy.setVelocityY(60);
            enemy.hitsToKill = 2;
        }
